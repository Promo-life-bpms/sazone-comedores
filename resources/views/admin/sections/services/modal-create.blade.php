<dialog id="my_modal_saludable" class="modal">
    <div class="modal-box space-y-3 px-8">
        ...
        <form method="POST" action="{{ route('storeService') }}" enctype="multipart/form-data" class="space-y-3">
            ...
            <div class="space-y-2">
                ...
                <input type="file" name="file_health" accept="image/*"
                    class="file-input bg-custom-blue file-input-bordered w-full @error('file_health') input-error @enderror" />
                @error('file_health')
                    <div class="text-red-500">
                        {{ $message }}
                    </div>
