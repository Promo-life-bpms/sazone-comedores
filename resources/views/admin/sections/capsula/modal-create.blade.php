<dialog id="my_modal_capnutri" class="modal">
    <div class="modal-box space-y-3 px-8">
        <form method="dialog">
            <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
        </form>
        <h3 class="font-bold text-lg text-center">Agregar Seccion de Capsulas</h3>
        <br>
        <form method="POST" action="{{ route('nutriCapsulas.store') }}" enctype="multipart/form-data" class="space-y-3">
            @method('POST')
            @csrf
            ...
            <div class="space-y-2">
                ...
                <input type="file" name="file_capsula" accept="image/*"
                    class="file-input bg-custom-blue file-input-bordered w-full @error('file_capsula') input-error @enderror" />
                @error('file_capsula')
                    <div class="text-red-500">
                        {{ $message }}
                    </div>
